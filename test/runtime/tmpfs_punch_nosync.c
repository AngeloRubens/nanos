...
static void *writer(void *arg)
{
    ...
    while (!stop) {
        ...
        /* No fsync in the tight loop: the sync path is where a context goes to
           sleep on the filesystem's lock, and that defect stops this before the
           pin and unpin of a punched range have had a chance to show anything.
           Left out so that what is under test here is the punch against the
           pages it takes away, not the commit. */
...
int main(int argc, char **argv)
{
    ...
    printf("tmpfs punch nosync test passed\n");
    return EXIT_SUCCESS;
}
