/* A shared mapping of a memory file should be able to be described with 2 MiB block
   mappings, and has to keep working when it cannot be.

   The kernel already maps anonymous memory that way -- every MAP_ANONYMOUS mapping
   asks for it, and the manifest only turns it off -- so a collector that keeps its
   heap in anonymous memory gets huge pages for free. ZGC does not: its heap is a
   memfd mapped MAP_SHARED, which is file-backed, and the file-backed fault path
   mapped one page at a time. This checks the two granularities and, just as much,
   that the huge one is never forced.

   mincore() is what makes the granularity visible from here: it reports a page as
   resident for every page a present entry covers, so one entry over 2 MiB shows up
   as 512 resident pages after a single touch, and a page at a time mapping shows up
   as one. Nothing else in the interface tells the two apart. */
...
static void test_shared_mapping_is_huge(void)
{
    ...
    base[0] = 0x5a;
    int n = resident(base, HUGE_SIZE);
    printf("shared aligned: %d of %d pages resident after one touch\n", n, HUGE_PAGES);
    test_assert(n == HUGE_PAGES);
...
static void test_unaligned_offset_falls_back(void)
{
    ...
    base[0] = 0x11;
    int n = resident(base, HUGE_SIZE);
    printf("shared unaligned offset: %d of %d pages resident after one touch\n",
           n, HUGE_PAGES);
    test_assert(n == 1);
...
static void test_private_mapping_stays_small(void)
{
    ...
    base[0] = 0x22;
    int n = resident(base, HUGE_SIZE);
    printf("private: %d of %d pages resident after one touch\n", n, HUGE_PAGES);
    test_assert(n == 1);
...
static void test_punch_inside_huge_window(void)
{
    ...
    int n = resident(base, HUGE_SIZE);
    printf("after punching one page of a huge window: %d of %d pages resident\n",
           n, HUGE_PAGES);
    test_assert(n < HUGE_PAGES);    /* the block entry cannot have survived */
...
static void test_partial_unmap_of_huge_window(void)
{
    ...
    int n = resident(base, HUGE_SIZE / 2);
    printf("after unmapping half a huge window: %d of %d pages resident\n",
           n, HUGE_PAGES / 2);
    test_assert(n == HUGE_PAGES / 2);
...
static void test_populated_window_falls_back(void)
{
    ...
    base[0] = 0x88;
    int n = resident(base, HUGE_SIZE);
    printf("window already holding a page: %d of %d pages resident\n", n, HUGE_PAGES);
    test_assert(n < HUGE_PAGES);
